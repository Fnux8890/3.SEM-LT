...
$(() => {
    ...
    $(document).on('click', '.mainContent .cardcontainer', function () {
        ...
        let soundfile = word.soundfile[Math.floor(Math.random() * word.soundfile.length)];
    ...
    $(document).on('click', '.answerOption', function (e) {
        ...
        if (mainWord.translation == aoText) {
            console.log(`CORRECT - word: ${mainWord.translation} == answerOption: ${aoText}`);
            colorChange.add({
                targets: this,
    ...
    function newCard() {
        ...
        for (let i = 0; i < 3; i++) {
            ...
            } while (wordIndex == cardIndex || answerOptions.includes(words[wordIndex]));
