...
function animateToDropzone(card, div) {
  ...
  const cardPos = { x: left, y: top };
  left = $(div).offset().left;
  top = $(div).offset().top;
  const dropPos = { x: left, y: top };
  const result = {
...
function animateCardOut(div) {
  ...
  const answerClass = $(div).attr('class');
  const t1 = anime.timeline({ targets: card });
...
function populateAnswers(data) {
  const answerA = `<p> ${data.answerOptions[0]} </p>`;
  const answerB = `<p> ${data.answerOptions[1]} </p>`;
...
function FromStackAnimation(card) {
  ...
  animationFromStack(card)
    .then(() => {
      audioPlayer.default.playWord(currentCard);
      interact(card)
        .draggable({
          listeners: {
            start() {
